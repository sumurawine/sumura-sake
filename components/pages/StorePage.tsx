      ...
      <div className="panel">
        ...
        {mode === 'area' ? (
          <div id="cat-nav" style={{ margin: '8px 0 2px', textAlign: 'center', lineHeight: 2 }}>
            <a href="#" style={{ margin: '0 6px', ...(cat === 'all' ? { color: plain ? 'var(--mx-gold)' : '#ffd479', fontWeight: 'bold' } : {}) }}
              onClick={(e) => { e.preventDefault(); setCat('all'); setSub('all'); }}>
              {brk(`${u('allCat', lang)} ${base.length}`)}
            </a>
            {ORDER.filter((c) => catTally.m[c]).map((c) => (
              <a key={c} href="#" style={{ margin: '0 6px', ...(cat === c ? { color: plain ? 'var(--mx-gold)' : '#ffd479', fontWeight: 'bold' } : {}) }}
